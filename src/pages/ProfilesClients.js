...
function ProfilesClients() {
  ...
  const notifyErr = () =>
    toast.error("من فضلك تاكد من كلمة السر الحالية", {
      position: "top-center",
      autoClose: 1000,
      hideProgressBar: false,
      closeOnClick: false,
  ...
  const formikPass = useFormik({
    ...
    validationSchema: yup.object().shape({
      ...
      newPassword: yup
        ...
        .matches(
          /^(?=.*[0-9])(?=.*[a-z]).{8,32}$/,
          "يجب ان تحتوي كلمة المرور عل حرف صغير وحرف كبير وان لاتقل عن 8 أحرف"
        )
    ...
    onSubmit: (values) => {
      ...
      axios
        .put("http://localhost:7000/client/changepassword", data, {
          headers: header,
        })
        .then((res) => {
          if (res.status == 200) {
            ...
            <div className="row">
              ...
              <div className="col-lg-8  col-sm-12">
                <div className="data_profile_client test">
                  ...
                  <div
                    className="child_pass_in_profile"
                    onClick={() => setOpenPass(true)}
                  >
                    <i class="fa-solid fa-key"></i>
                  </div>
                  <div>عدد المنشورات ( {Profile?.jobs?.length})</div>
                </div>
              </div>
            </div>
          )}
            ...
            <div>
              <Typography id="modal-desc" textColor="text.tertiary">
                ...
                >
                  ...
                  >
                    {formikPass.touched.confirmPass &&
                      formikPass.errors.confirmPass}
                  </small>
                  <div className="w-100 mt-3 text-start">
